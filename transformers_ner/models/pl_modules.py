...
class NERModule(pl.LightningModule):
    ...
    def compute_f1_score(
        ...
        predictions = [
            [p for p in preds[:length]]
            for preds, length in zip(true_predictions, sentence_lengths)
        ]
        # same for labels
        #labels[labels == -100] = 0
        if isinstance(labels, torch.Tensor):
        ...
        labels = [
            [l for l in label[:length]]
            for label, length in zip(true_labels, sentence_lengths)
        ]
        # return scores
